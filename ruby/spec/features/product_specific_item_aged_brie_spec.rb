...
describe ItemIncreasingQuality do
  context 'general cases using Aged Brie' do
    subject(:agedBrie) { described_class.new('Aged Brie', 2, 0) }
    let(:items) { [agedBrie] }
    ...
    it 'expect sell_in/quality to be -24/50 after 25th update' do
  ...
  context 'general cases using Aged Brie - edge case' do
    subject(:agedBrie) { described_class.new('Aged Brie', 0, 49) }
    let(:items) { [agedBrie] }
    let(:gilded_rose) { GildedRose.new items }
