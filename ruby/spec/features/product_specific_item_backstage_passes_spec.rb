...
describe ItemEscalatingQuality do
  ...
  context 'Quality increases by 2 when there are 10 days or less' do
    subject(:backstage) { described_class.new('Backstage passes to a TAFKAL80ETC concert', 10, 0) }
    let(:items) { [backstage] }
    let(:gilded_rose) { GildedRose.new items }
  ...
  context 'Quality increases by 3 when there are 5 days or less' do
    subject(:backstage) { described_class.new('Backstage passes to a TAFKAL80ETC concert', 5, 0) }
    let(:items) { [backstage] }
    let(:gilded_rose) { GildedRose.new items }
  ...
  context 'Quality drops to 0 after the concert' do
    subject(:backstage) { described_class.new('Backstage passes to a TAFKAL80ETC concert', 1, 4) }
    let(:items) { [backstage] }
    let(:gilded_rose) { GildedRose.new items }
